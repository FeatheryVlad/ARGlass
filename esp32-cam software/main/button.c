...
typedef struct {
  ...
  uint32_t ltime_duration;
  uint32_t ltime_repeat;
} debounce_t;
...
void set_long_press_duration(uint32_t time){
  debounce->ltime_duration = time;
}

void set_long_press_repeat(uint32_t time){
  debounce->ltime_repeat = time;
}

QueueHandle_t gpio_buttons_init(unsigned long long pin_select, gpio_pull_mode_t pull_mode)
{
  if (pin_count != -1) {
  ...
  for (int pin=0; pin<=39; pin++) {
    if ((1ULL<<pin) & pin_select) {
      ...
      debounce[idx].ltime_duration = 600;
      debounce[idx].ltime_repeat = 150;
